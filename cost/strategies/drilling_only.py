"""Стратегия: только буровые работы."""
...
from cost.strategies.common import finalize_aggregate, run_drilling_module, run_fixed_module, run_labor_module


class DrillingOnlyScenarioStrategy(BaseScenarioStrategy):
    scenario_id = "drilling"
    ...
    def calculate(
        ...
        **kwargs: Any,
        ...
        if block_data is None:
            ...
            result.notes.append("Нет данных блока для расчёта бурения.")
            return result
        ...
        run_drilling_module(result, block_data, ctx)
