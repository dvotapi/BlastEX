...
def _bit_diameters(sections: dict[str, list[ReferenceItem]]) -> set[Decimal]:
    ...
    for condition in sections["drilling_conditions"]:
        ...
        try:
            ...
            continue
        # Диаметр как есть, без `normalize()`: тот округляет число длиннее
        # 28 значащих цифр, и проверка ревизии не нашла бы коэффициента для
        # исходного диаметра коронки. «171.0» и «171» множество и так
        # считает одним значением.
        found.add(diameter)
    return found
