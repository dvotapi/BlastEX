...
class LaborRatePayload(ReferencePayload):
    ...
    piece_rate_rub: Decimal = UnitField(
        ...
        description=(
            "Начисляется каждому в смене: рубли за столько единиц драйвера сдельной оплаты, "
            "сколько указано у должности в поле «За единиц драйвера». "
            "Без драйвера у должности не начисляется"
        ),
        default=Decimal("0"),
    )
