...
class DrillingDifficultyPayload(ReferencePayload):
    ...
    hardness: list[HardnessBand] = Field(
        ...
        description=(
            "Интервалы крепости f по возрастанию, без разрывов: нижняя граница строки не включается, "
            "верхняя включается; база f = 10 → коэффициент 1. Первая строка без нижней границы, последняя без верхней"
        ),
    )
    diameter: list[DiameterFactor] = Field(
        default_factory=list,
        title="Диаметр коронки",
        description="Коэффициент на каждый диаметр коронок из условий бурения; база Ø 152 мм → коэффициент 1",
    )
