...
function FullBvrCalc({
  ...
  useEffect(() => {
    ...
    setOptimizeError(null);
    setSheetLoadError("");
    // Пересчёт по правке настроек прежнего объекта остановится сам (он сверяет
    // номер загрузки), но флаг снял бы только когда вернётся его запрос — а у
    // запросов нет таймаута, и новый объект мог бы навсегда остаться с
    // «Пересчёт…». Новая правка настроек поставит флаг заново.
