...
function FactCell({
  ...
  // Значение сменилось снаружи (например, загрузили другой лист) — показываем
  // новое; свой незаконченный ввод не трогаем. Удаление строки сюда не
  // относится: у строк стабильные ключи (`rowIds` ниже), поэтому при удалении
  // React размонтирует ячейку вместе со строкой, а не переиспользует её для
  // соседних данных.
  useEffect(() => {
