 ...
 * нельзя, а ошибка, если есть, осталась от прошлой попытки (запрос без коронок
 * не уходит и ошибку не сбрасывает). Расчёт упал — ведём к сообщению об ошибке:
 * причина бывает любой (сеть, сервер, настройки), а сообщения сервера сами
 * говорят, что менять. Иначе — пересчитать.
 */
