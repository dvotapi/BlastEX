...
export function useCalcInputsAutosave({
   ...
   *
   * Пока мы ждём очередь, именно этот незавершённый PUT может ответить
   * ошибкой — тогда его `catch` вернёт правку обратно в `pendingRef`
   * (`pendingAfterSaveError`) уже ПОСЛЕ того, как мы здесь решили, что
   * отправлять больше нечего (мы сняли и обнулили `pendingRef` ДО этого
   * ожидания). Без повторной проверки эта правка осталась бы в `pendingRef`
   * под именем прежнего объекта и никогда не отправилась бы: следующий же
   * рендер (уже с новым объектом) перезапишет её отсечкой нового листа.
   * Поэтому после ожидания смотрим `pendingRef` ещё раз и, если запись
   * успела туда вернуться, досылаем её тем же путём. Одного дополнительного
   * круга достаточно для этой гонки (тайм-код успевает ответить максимум
   * один раз до того, как мы досюда дойдём); если и повторная запись
   * упадёт, дальше действует обычная логика лучшего эффекта — как и раньше
   * при обрыве сети. */
  const flush = useCallback(async () => {
    ...
    // TS сужает тип `pendingRef.current` до `null` по предыдущему присвоению
    // выше, не учитывая, что вызов `save()` внутри `catch` мог положить туда
    // значение заново, — явно снимаем сужение.
    const requeued = pendingRef.current as PendingSave | null;
    if (requeued) {
      pendingRef.current = null;
      save(requeued.objectName, requeued.inputs);
      await queueRef.current;
    }
  }, [save]);
