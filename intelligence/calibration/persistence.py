"""Write-once storage for residual-correction artifacts.

Artifacts live in ``data/teams/{team_id}/calibration/`` — never inside
...
class CalibrationNotFoundError(Exception):
    """Calibration model with the given id is not in the team store."""


class ImmutableCalibrationError(Exception):
...
def calibration_dir(team_id: str) -> Path:
    return team_dir(team_id) / "calibration"


def _validate_model_id(model_id: str) -> None:
    if not model_id or model_id != Path(model_id).name or model_id in {".", ".."}:
        raise CalibrationNotFoundError(f"Модель калибровки «{model_id}» не найдена.")


def metadata_path(team_id: str, model_id: str) -> Path:
    _validate_model_id(model_id)
    base = calibration_dir(team_id).resolve()
...
def list_models(team_id: str) -> list[CalibrationSummary]:
...
def load_model(team_id: str, model_id: str) -> CalibrationModel:
    ...
    if stored_hash and stored_hash != integrity_hash(data):
        raise ImmutableCalibrationError(
    ...
    if stored_artifact and stored_artifact != artifact_hash(artifact):
        raise ImmutableCalibrationError(f"Артефакт модели «{model_id}» повреждён.")
    estimator = joblib.load(artifact)
    return CalibrationModel.from_dict(data, estimator=estimator)


def save_model(team_id: str, model: CalibrationModel) -> CalibrationModel:
...
def production_model(team_id: str, site_id: str, model_type: str) -> CalibrationModel | None:
    matches = [
        item
        for item in list_models(team_id)
...
def set_status(team_id: str, model_id: str, status: str) -> CalibrationModel:
