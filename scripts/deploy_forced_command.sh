...
# стал неотслеживаемым только сейчас. По умолчанию checkout молча
# перезаписывает игнорируемый файл, который $sha начал отслеживать, и
# локальное значение прода (.env и т. п.) пропало бы; без перезаписи git
# отказывается, и рабочая копия остаётся как была.
ensure_clean
if ! git checkout --quiet --no-overwrite-ignore --detach "$sha"; then
  reject "checkout $sha затёр бы файлы сервера (список выше) — перенесите их и повторите"
fi
ensure_clean
